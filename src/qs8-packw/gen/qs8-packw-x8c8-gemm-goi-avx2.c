...
void xnn_qs8_packw_gemm_goi_ukernel_x8c8__avx2(
  size_t g,
  size_t nc,
  size_t kc,
  size_t nr,
  ...
  do {
    ...
    for (;n >= 16; n -= 16) {
      ...
      if XNN_LIKELY(b != NULL) {
        ...
        ((int32_t*) out)[7] = b[7];
        b += 8;
        ((int32_t*) out)[64 + 0] = b[0];
        ((int32_t*) out)[64 + 1] = b[1];
        ((int32_t*) out)[64 + 2] = b[2];
        ((int32_t*) out)[64 + 3] = b[3];
        ((int32_t*) out)[64 + 4] = b[4];
        ((int32_t*) out)[64 + 5] = b[5];
        ((int32_t*) out)[64 + 6] = b[6];
        ((int32_t*) out)[64 + 7] = b[7];
        b += 8;
      } else {
        ((int32_t*) out)[0] = 0;
        ((int32_t*) out)[1] = 0;
      ...
      size_t k = kc;
      __m256i vksum, vksum2;
      for (; k >= 8; k -= 8) {
        __m256i v0 = _mm256_castsi128_si256(_mm_loadl_epi64((const __m128i *)w0));
        __m256i v1 = _mm256_castsi128_si256(_mm_loadl_epi64((const __m128i *)(w0 + 8)));
        v0 = _mm256_insert_epi64(v0, *(int64_t *)w1, 1);
        v0 = _mm256_insert_epi64(v0, *(int64_t *)w2, 2);
        v0 = _mm256_insert_epi64(v0, *(int64_t *)w3, 3);
        v1 = _mm256_insert_epi64(v1, *(int64_t *)(w1 + 8), 1);
        v1 = _mm256_insert_epi64(v1, *(int64_t *)(w2 + 8), 2);
        v1 = _mm256_insert_epi64(v1, *(int64_t *)(w3 + 8), 3);

        vksum = _mm256_sad_epu8(v0, kzero);
        vksum2 = _mm256_sad_epu8(v1, kzero);
        // ksum0 += _mm_cvtsi128_si32(_mm256_sad_epu8(v0, kzero));
        ksum0 += _mm256_extract_epi32(vksum, 0) + _mm256_extract_epi32(vksum2, 0);
        ksum1 += _mm256_extract_epi32(vksum, 2) + _mm256_extract_epi32(vksum2, 2);
        ksum2 += _mm256_extract_epi32(vksum, 4) + _mm256_extract_epi32(vksum2, 4);
        ksum3 += _mm256_extract_epi32(vksum, 6) + _mm256_extract_epi32(vksum2, 6);
        
        // _mm256_dpbssd_epi32(vacc, v0, 0);

        _mm256_storeu_si256((__m256i *)&out[0],  v0);
        _mm256_storeu_si256((__m256i *)&out[64],  v1);
        ...
        __m256i v5 = _mm256_castsi128_si256(_mm_loadl_epi64((const __m128i *)(w0 + 8)));
        v4 = _mm256_insert_epi64(v4, *(int64_t *)w5, 1);
        v4 = _mm256_insert_epi64(v4, *(int64_t *)w6, 2);
        v4 = _mm256_insert_epi64(v4, *(int64_t *)w7, 3);
        v5 = _mm256_insert_epi64(v5, *(int64_t *)(w5 + 8), 1);
        v5 = _mm256_insert_epi64(v5, *(int64_t *)(w6 + 8), 2);
        v5 = _mm256_insert_epi64(v5, *(int64_t *)(w7 + 8), 3);

        vksum = _mm256_sad_epu8(v4, kzero);
        vksum2 = _mm256_sad_epu8(v5, kzero);
        // ksum4 += _mm_cvtsi128_si32(_mm256_sad_epu8(v4, kzero));
        ksum4 += _mm256_extract_epi32(vksum, 0) + _mm256_extract_epi32(vksum2, 0);
        ksum5 += _mm256_extract_epi32(vksum, 2) + _mm256_extract_epi32(vksum2, 2);
        ksum6 += _mm256_extract_epi32(vksum, 4) + _mm256_extract_epi32(vksum2, 4);
        ksum7 += _mm256_extract_epi32(vksum, 6) + _mm256_extract_epi32(vksum2, 6);
        
        // _mm256_dpbssd_epi32(vacc, v4, 0);

        _mm256_storeu_si256((__m256i *)&out[32],  v4);
        _mm256_storeu_si256((__m256i *)&out[96],  v4);
    ...
    for (;n >= 8; n -= 8) {
      ...
      for (; k >= 8; k -= 8) {
        ...
        vksum = _mm256_sad_epu8(v0, kzero);
        // ksum0 += _mm_cvtsi128_si32(_mm256_sad_epu8(v0, kzero));
        ksum0 += _mm256_extract_epi32(vksum, 0);
        ksum1 += _mm256_extract_epi32(vksum, 2);
        ksum2 += _mm256_extract_epi32(vksum, 4);
        ksum3 += _mm256_extract_epi32(vksum, 6);
        ...
        vksum = _mm256_sad_epu8(v4, kzero);
        // ksum4 += _mm_cvtsi128_si32(_mm256_sad_epu8(v4, kzero));
        ksum4 += _mm256_extract_epi32(vksum, 0);
        ksum5 += _mm256_extract_epi32(vksum, 2);
        ksum6 += _mm256_extract_epi32(vksum, 4);
        ksum7 += _mm256_extract_epi32(vksum, 6);
